...
public class UserController {
	...
	// yml파일에 만든 키 => 노출되면 안되며 노출되면 모든 오워스 로그인 비밀번호를 알수가 있다.
	@Value("${cos.key}")
	...
	public String kakaoCallback(String code) { // 쿼리스트링 키값으로 code를 받아서 바로 매핑이 가능하다.
...
//		System.out.println("블로그서버 비밀번호 : " + cosKey);
		...
		if (originUser.getUsername() == null) {
			System.out.println("기존 회원이 아니기 때문에 자동으로 회원가입이 진행됩니다.");
